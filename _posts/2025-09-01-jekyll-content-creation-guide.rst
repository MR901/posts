...
- **Extension matters** - ``.md`` or ``.markdown`` (native); ``.rst`` requires plugin
- **Case sensitive** - use lowercase for consistency

**Note:** reStructuredText (``.rst``) support requires the ``jekyll-rst`` plugin and is not available on GitHub Pages by default.
...
- ``title`` - Post title (recommended; if omitted, Jekyll derives from filename)
- ``date`` - Publication date with timezone (can override filename date)
- ``layout`` - Template to use (typically ``post`` for blog posts)
...
::
    ...
    allow_edit: false        # Custom field (theme-specific, for editor hints)
    ---

**Note:** The ``allow_edit`` field is a custom feature (not standard Jekyll) that can be used with editor configurations to protect finalized posts. Pair it with ``.cursorignore`` entries to prevent accidental edits.
...
- **Consistent naming** - Choose lowercase or title case and stick with it
