...
if command -v python3 >/dev/null 2>&1; then
  ...
  python3 -m pip install --no-cache-dir docutils pygments >/dev/null 2>&1 || true
fi

# Fallback: install Python tooling via APT if pip is missing
if ! command -v pip3 >/dev/null 2>&1; then
  sudo apt-get update -y >/dev/null 2>&1 || true
  sudo apt-get install -y python3-pip python3-docutils python3-pygments >/dev/null 2>&1 || true
fi
